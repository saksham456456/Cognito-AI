...
const MessageComponent: React.FC<MessageProps> = ({ message, isLastMessage, onCopy, onSpeak, onRegenerate, speakingMessageId }) => {
  ...
  const bubbleClasses = `max-w-xl px-4 py-3 rounded-2xl relative ${
    isUser
      ? 'bg-primary dark:bg-yellow-500 text-primary-foreground dark:text-black rounded-br-none'
      : 'bg-card dark:bg-[#1f1f1f] text-card-foreground dark:text-gray-200 rounded-bl-none border border-card-border dark:border-[#333]'
  }`;
  ...
  const MessageActions = () => (
    <div className="absolute -bottom-2 right-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
      <button onClick={handleCopy} className="p-1.5 rounded-full bg-input hover:bg-input-border dark:bg-[#292929] dark:hover:bg-[#404040] transition-colors">
      ...
      <button onClick={() => onSpeak(message)} className="p-1.5 rounded-full bg-input hover:bg-input-border dark:bg-[#292929] dark:hover:bg-[#404040] transition-colors">
        <Volume2Icon className={`w-4 h-4 ${isSpeaking ? 'text-primary dark:text-yellow-400' : 'text-card-foreground/60 dark:text-gray-400'}`} />
      </button>
      {isLastMessage && (
        <button onClick={onRegenerate} className="p-1.5 rounded-full bg-input hover:bg-input-border dark:bg-[#292929] dark:hover:bg-[#404040] transition-colors">
  ...
  return (
    <div className={containerClasses}>
      {!isUser && (
        <div className="flex-shrink-0 h-8 w-8 rounded-full bg-card dark:bg-[#1f1f1f] flex items-center justify-center border border-card-border dark:border-[#333]">
