...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const ModeButton: React.FC<{
      ...
      <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold transition-colors ${
              isActive
                  ? 'bg-primary/20 dark:bg-yellow-400/10 text-primary dark:text-yellow-400'
                  : 'text-card-foreground/70 dark:text-gray-400 hover:bg-input dark:hover:bg-[#292929]'
          }`}
      >
          {children}
      </button>
  ...
  return (
    <aside className={`absolute md:relative z-20 flex-shrink-0 w-80 bg-card dark:bg-[#1f1f1f] border-r border-card-border dark:border-zinc-800 flex flex-col transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
      ...
      <div className="p-2 border-b border-card-border dark:border-zinc-800">
        <div className="flex items-center gap-2 p-1 bg-input dark:bg-[#292929] rounded-xl">
      ...
      {currentView === 'chat' && (
        <>
            <div className="p-4 flex flex-col gap-4">
                <button onClick={onNewChat} className="w-full flex items-center justify-center gap-2 p-2 rounded-lg hover:bg-input dark:hover:bg-[#292929] transition-colors border border-card-border dark:border-zinc-700 text-outline-sm font-semibold">
          ...
          <ul className="space-y-1 mt-2">
              ...
              <li>
                  <button
                      ...
                      disabled={!activeChatId || currentView !== 'chat'}
              ...
              <li>
                  <button
                      ...
                      disabled={chats.length === 0 || currentView !== 'chat'}
